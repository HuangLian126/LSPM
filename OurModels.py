...
class LSPMLayer(Module)
    def __init__(self, in_planes, S)
        ...
        self.conv     = nn.Conv2d(in_channels=in_planes, out_channels=SS, kernel_size=1, bias=False)

    def forward(self, x)

        B, C, H, W = x.size()

        x_reshape_T = x.view(B, -1, H  W).permute(0, 2, 1)           # BHWC
        x_reshape   = x.view(B, -1, H  W)                            # BCHW
        MM1         = torch.bmm(x_reshape_T, x_reshape)               # BHWHW
        MM1         = F.softmax(MM1, dim=-1)                          # BHWHW
        ...
        GAP = GAP.view(B, -1, self.Sself.S)                          # BC(SS)

        MM3     = torch.bmm(GAP, MM2).view(B, C, H, W)                # BCHW
...
class LSPM(Module)
    def __init__(self, in_planes)
        ...
        self.conv  = nn.Conv2d(5self.in_planes, self.in_planes, 1, 1, bias=False)

    def forward(self, x)
        
            inputs 
                x  input feature maps(BCHW)
            returns 
             out  BCHW
